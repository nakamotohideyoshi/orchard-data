const path = require('path')

module.exports = {

  caseSensitiveSearch: {
    'metadata': {
      'source': path.resolve(__dirname, '..', '..', 'data-tests', 'input-files', 'categories', 'test.tsv'),
      'artist_blacklist': 'test',
      'keyword_blacklist': 'abcd',
      'duplicates_threshold': 1,
      'various_artists_threshold': 0,
      'lang': 'en-US',
      'status': 1,
      'time': 1519073116716
    },
    'dataset': [
      {
        'release_name': 'abcd',
        'release_meta_language': 'English',
        'orchard_artist': 'Gay and Terry Woods',
        'artist_country': 'United Kingdom',
        'subaccount_name': '',
        'artist_url': '',
        'release_artists_primary_artist': 'Gay and Terry Woods',
        'release_artists_featuring': '',
        'release_artists_remixer': '',
        'release_artists_composer': '',
        'release_artists_orchestra': '',
        'release_artists_ensemble': '',
        'release_artists_conductor': '',
        'release_date': '1/1/1978',
        'sale_start_date': '11/15/2016',
        'itunes_preorder': '',
        'itunes_preorder_date': '',
        'preorder_preview': '',
        'release_itunes_pricing': 'Mid/Front',
        'release_amazon_pricing': 'Frontline',
        'format': 'Full Length',
        'imprint': 'Cooking Vinyl',
        'genre': 'Folk',
        'sub_genre': 'Folk',
        'copyright_information': '1978 Cooking Vinyl',
        'digital_upc': '',
        'manufacturers_upc': '711297201925',
        'label_catalog_number': 'GUMBOCD019',
        'release_version': '',
        'file_name': '711297201925_1_1.wav',
        'volume': '1',
        'track_no': '1',
        'track_name': 'We Can Work This One Out',
        'meta_language': 'English',
        'version': '',
        'track_artist': 'Gay and Terry Woods',
        'track_artist_featuring': '',
        'track_artist_remixer': '',
        'track_artist_composer': '',
        'track_artist_orchestra': '',
        'track_artist_ensemble': '',
        'track_artist_conductor': '',
        'track_itunes_pricing': 'Front',
        'track_amazon_pricing': 'Frontline',
        'explicit': 'No',
        'isrc': 'GBCEJ7800024',
        'third_party_publisher': 'No',
        'p_information': '1978 Cooking Vinyl',
        'songwriters': '0',
        'publishers': '',
        'only_include_exclude': 'Only Include',
        'territories': 'DZ|AO|BJ|BW|BF|BI|CM|CV|CF|TD|KM|CG|CD|CI|DJ|EG|GQ|ER|ET|GA|GM|GH|GN|GW|KE|LS|LR|LY|MG|MW|ML|MU|MR|MA|MZ|NA|NE|NG|RW|ST|SN|SL|SC|SO|ZA|SZ|SD|TZ|TG|TN|UG|ZM|EH|ZW|AQ|BV|TF|HM|GS|AF|AZ|BH|BD|BT|IO|BN|KH|CN|CX|CC|HK|CC|IN|ID|IR|IQ|JP|JO|KW|LA|LB|MO|MY|MV|MN|MM|NP|KP|OM|PK|PS|PH|QA|SA|SG|KR|LK|SY|TW|TH|TL|AE|VN|YE|AX|AL|AD|AM|AT|BY|BE|BA|BG|HR|CY|CZ|DK|EE|FO|FI|FR|GE|DE|GL|GI|GR|GG|VA|HU|IS|IE|IM|IL|IT|JE|KZ|KG|LV|LI|LT|LU|MK|MT|YT|MD|MC|NL|NO|PL|PT|RE|RO|RU|SH|SM|CS|SK|SI|ES|SJ|SE|CH|TJ|TR|TM|UA|GB|UZ|AI|AG|AW|BS|BZ|BB|BM|CA|KY|CR|CU|DM|DO|SV|GD|GP|HT|GT|HN|JM|MQ|MX|UM|MS|NI|AN|PA|PR|KN|LC|PM|VC|TT|TC|US|VG|VI|AS|AU|CK|FJ|PF|GU|KI|MH|FM|NR|NC|NZ|NU|NF|MP|PW|PG|PN|WS|SB|TK|TO|VU|TV|WF|AR|BO|BR|CL|CO|EC|FK|GF|GY|PY|PE|SR|UY|VE'
      }
    ]
  },

  caseInSensitiveSearch: {
    'metadata': {
      'source': path.resolve(__dirname, '..', '..', 'data-tests', 'input-files', 'categories', 'test.tsv'),
      'artist_blacklist': 'test',
      'keyword_blacklist': 'abcd',
      'duplicates_threshold': 1,
      'various_artists_threshold': 0,
      'lang': 'en-US',
      'status': 1,
      'time': 1519073116716
    },
    'dataset': [
      {
        'release_name': 'ABCD',
        'release_meta_language': 'English',
        'orchard_artist': 'Gay and Terry Woods',
        'artist_country': 'United Kingdom',
        'subaccount_name': '',
        'artist_url': '',
        'release_artists_primary_artist': 'Gay and Terry Woods',
        'release_artists_featuring': '',
        'release_artists_remixer': '',
        'release_artists_composer': '',
        'release_artists_orchestra': '',
        'release_artists_ensemble': '',
        'release_artists_conductor': '',
        'release_date': '1/1/1978',
        'sale_start_date': '11/15/2016',
        'itunes_preorder': '',
        'itunes_preorder_date': '',
        'preorder_preview': '',
        'release_itunes_pricing': 'Mid/Front',
        'release_amazon_pricing': 'Frontline',
        'format': 'Full Length',
        'imprint': 'Cooking Vinyl',
        'genre': 'Folk',
        'sub_genre': 'Folk',
        'copyright_information': '1978 Cooking Vinyl',
        'digital_upc': '',
        'manufacturers_upc': '711297201925',
        'label_catalog_number': 'GUMBOCD019',
        'release_version': '',
        'file_name': '711297201925_1_2.wav',
        'volume': '1',
        'track_no': '2',
        'track_name': 'Friends of Mine',
        'meta_language': 'English',
        'version': '',
        'track_artist': 'Gay and Terry Woods',
        'track_artist_featuring': '',
        'track_artist_remixer': '',
        'track_artist_composer': '',
        'track_artist_orchestra': '',
        'track_artist_ensemble': '',
        'track_artist_conductor': '',
        'track_itunes_pricing': 'Front',
        'track_amazon_pricing': 'Frontline',
        'explicit': 'No',
        'isrc': 'GBCEJ7800025',
        'third_party_publisher': 'No',
        'p_information': '1978 Cooking Vinyl',
        'songwriters': '0',
        'publishers': '',
        'only_include_exclude': 'Only Include',
        'territories': 'DZ|AO|BJ|BW|BF|BI|CM|CV|CF|TD|KM|CG|CD|CI|DJ|EG|GQ|ER|ET|GA|GM|GH|GN|GW|KE|LS|LR|LY|MG|MW|ML|MU|MR|MA|MZ|NA|NE|NG|RW|ST|SN|SL|SC|SO|ZA|SZ|SD|TZ|TG|TN|UG|ZM|EH|ZW|AQ|BV|TF|HM|GS|AF|AZ|BH|BD|BT|IO|BN|KH|CN|CX|CC|HK|CC|IN|ID|IR|IQ|JP|JO|KW|LA|LB|MO|MY|MV|MN|MM|NP|KP|OM|PK|PS|PH|QA|SA|SG|KR|LK|SY|TW|TH|TL|AE|VN|YE|AX|AL|AD|AM|AT|BY|BE|BA|BG|HR|CY|CZ|DK|EE|FO|FI|FR|GE|DE|GL|GI|GR|GG|VA|HU|IS|IE|IM|IL|IT|JE|KZ|KG|LV|LI|LT|LU|MK|MT|YT|MD|MC|NL|NO|PL|PT|RE|RO|RU|SH|SM|CS|SK|SI|ES|SJ|SE|CH|TJ|TR|TM|UA|GB|UZ|AI|AG|AW|BS|BZ|BB|BM|CA|KY|CR|CU|DM|DO|SV|GD|GP|HT|GT|HN|JM|MQ|MX|UM|MS|NI|AN|PA|PR|KN|LC|PM|VC|TT|TC|US|VG|VI|AS|AU|CK|FJ|PF|GU|KI|MH|FM|NR|NC|NZ|NU|NF|MP|PW|PG|PN|WS|SB|TK|TO|VU|TV|WF|AR|BO|BR|CL|CO|EC|FK|GF|GY|PY|PE|SR|UY|VE'
      }
    ]
  },

  firstInKeywrodblacklist: {
    'metadata': {
      'source': path.resolve(__dirname, '..', '..', 'data-tests', 'input-files', 'categories', 'test.tsv'),
      'artist_blacklist': 'test',
      'keyword_blacklist': 'second third',
      'duplicates_threshold': 1,
      'various_artists_threshold': 0,
      'lang': 'en-US',
      'status': 1,
      'time': 1519073116716
    },
    'dataset': [
      {
        'release_name': 'second',
        'release_meta_language': 'English',
        'orchard_artist': 'Gay and Terry Woods',
        'artist_country': 'United Kingdom',
        'subaccount_name': '',
        'artist_url': '',
        'release_artists_primary_artist': 'Gay and Terry Woods',
        'release_artists_featuring': '',
        'release_artists_remixer': '',
        'release_artists_composer': '',
        'release_artists_orchestra': '',
        'release_artists_ensemble': '',
        'release_artists_conductor': '',
        'release_date': '1/1/1978',
        'sale_start_date': '11/15/2016',
        'itunes_preorder': '',
        'itunes_preorder_date': '',
        'preorder_preview': '',
        'release_itunes_pricing': 'Mid/Front',
        'release_amazon_pricing': 'Frontline',
        'format': 'Full Length',
        'imprint': 'Cooking Vinyl',
        'genre': 'Folk',
        'sub_genre': 'Folk',
        'copyright_information': '1978 Cooking Vinyl',
        'digital_upc': '',
        'manufacturers_upc': '711297201925',
        'label_catalog_number': 'GUMBOCD019',
        'release_version': '',
        'file_name': '711297201925_1_3.wav',
        'volume': '1',
        'track_no': '3',
        'track_name': 'Full Moon',
        'meta_language': 'English',
        'version': '',
        'track_artist': 'Gay and Terry Woods',
        'track_artist_featuring': '',
        'track_artist_remixer': '',
        'track_artist_composer': '',
        'track_artist_orchestra': '',
        'track_artist_ensemble': '',
        'track_artist_conductor': '',
        'track_itunes_pricing': 'Front',
        'track_amazon_pricing': 'Frontline',
        'explicit': 'No',
        'isrc': 'GBCEJ7800026',
        'third_party_publisher': 'No',
        'p_information': '1978 Cooking Vinyl',
        'songwriters': '0',
        'publishers': '',
        'only_include_exclude': 'Only Include',
        'territories': 'DZ|AO|BJ|BW|BF|BI|CM|CV|CF|TD|KM|CG|CD|CI|DJ|EG|GQ|ER|ET|GA|GM|GH|GN|GW|KE|LS|LR|LY|MG|MW|ML|MU|MR|MA|MZ|NA|NE|NG|RW|ST|SN|SL|SC|SO|ZA|SZ|SD|TZ|TG|TN|UG|ZM|EH|ZW|AQ|BV|TF|HM|GS|AF|AZ|BH|BD|BT|IO|BN|KH|CN|CX|CC|HK|CC|IN|ID|IR|IQ|JP|JO|KW|LA|LB|MO|MY|MV|MN|MM|NP|KP|OM|PK|PS|PH|QA|SA|SG|KR|LK|SY|TW|TH|TL|AE|VN|YE|AX|AL|AD|AM|AT|BY|BE|BA|BG|HR|CY|CZ|DK|EE|FO|FI|FR|GE|DE|GL|GI|GR|GG|VA|HU|IS|IE|IM|IL|IT|JE|KZ|KG|LV|LI|LT|LU|MK|MT|YT|MD|MC|NL|NO|PL|PT|RE|RO|RU|SH|SM|CS|SK|SI|ES|SJ|SE|CH|TJ|TR|TM|UA|GB|UZ|AI|AG|AW|BS|BZ|BB|BM|CA|KY|CR|CU|DM|DO|SV|GD|GP|HT|GT|HN|JM|MQ|MX|UM|MS|NI|AN|PA|PR|KN|LC|PM|VC|TT|TC|US|VG|VI|AS|AU|CK|FJ|PF|GU|KI|MH|FM|NR|NC|NZ|NU|NF|MP|PW|PG|PN|WS|SB|TK|TO|VU|TV|WF|AR|BO|BR|CL|CO|EC|FK|GF|GY|PY|PE|SR|UY|VE'
      }
    ]
  },
  lastInKeywrodblacklist: {
    'metadata': {
      'source': path.resolve(__dirname, '..', '..', 'data-tests', 'input-files', 'categories', 'test.tsv'),
      'artist_blacklist': 'test',
      'keyword_blacklist': 'second third',
      'duplicates_threshold': 1,
      'various_artists_threshold': 0,
      'lang': 'en-US',
      'status': 1,
      'time': 1519073116716
    },
    'dataset': [
      {
        'release_name': 'third',
        'release_meta_language': 'English',
        'orchard_artist': 'Gay and Terry Woods',
        'artist_country': 'United Kingdom',
        'subaccount_name': '',
        'artist_url': '',
        'release_artists_primary_artist': 'Gay and Terry Woods',
        'release_artists_featuring': '',
        'release_artists_remixer': '',
        'release_artists_composer': '',
        'release_artists_orchestra': '',
        'release_artists_ensemble': '',
        'release_artists_conductor': '',
        'release_date': '1/1/1978',
        'sale_start_date': '11/15/2016',
        'itunes_preorder': '',
        'itunes_preorder_date': '',
        'preorder_preview': '',
        'release_itunes_pricing': 'Mid/Front',
        'release_amazon_pricing': 'Frontline',
        'format': 'Full Length',
        'imprint': 'Cooking Vinyl',
        'genre': 'Folk',
        'sub_genre': 'Folk',
        'copyright_information': '1978 Cooking Vinyl',
        'digital_upc': '',
        'manufacturers_upc': '711297201925',
        'label_catalog_number': 'GUMBOCD019',
        'release_version': '',
        'file_name': '711297201925_1_4.wav',
        'volume': '1',
        'track_no': '4',
        'track_name': "I've a Lady",
        'meta_language': 'English',
        'version': '',
        'track_artist': 'Gay and Terry Woods',
        'track_artist_featuring': '',
        'track_artist_remixer': '',
        'track_artist_composer': '',
        'track_artist_orchestra': '',
        'track_artist_ensemble': '',
        'track_artist_conductor': '',
        'track_itunes_pricing': 'Front',
        'track_amazon_pricing': 'Frontline',
        'explicit': 'No',
        'isrc': 'GBCEJ7800027',
        'third_party_publisher': 'No',
        'p_information': '1978 Cooking Vinyl',
        'songwriters': '0',
        'publishers': '',
        'only_include_exclude': 'Only Include',
        'territories': 'DZ|AO|BJ|BW|BF|BI|CM|CV|CF|TD|KM|CG|CD|CI|DJ|EG|GQ|ER|ET|GA|GM|GH|GN|GW|KE|LS|LR|LY|MG|MW|ML|MU|MR|MA|MZ|NA|NE|NG|RW|ST|SN|SL|SC|SO|ZA|SZ|SD|TZ|TG|TN|UG|ZM|EH|ZW|AQ|BV|TF|HM|GS|AF|AZ|BH|BD|BT|IO|BN|KH|CN|CX|CC|HK|CC|IN|ID|IR|IQ|JP|JO|KW|LA|LB|MO|MY|MV|MN|MM|NP|KP|OM|PK|PS|PH|QA|SA|SG|KR|LK|SY|TW|TH|TL|AE|VN|YE|AX|AL|AD|AM|AT|BY|BE|BA|BG|HR|CY|CZ|DK|EE|FO|FI|FR|GE|DE|GL|GI|GR|GG|VA|HU|IS|IE|IM|IL|IT|JE|KZ|KG|LV|LI|LT|LU|MK|MT|YT|MD|MC|NL|NO|PL|PT|RE|RO|RU|SH|SM|CS|SK|SI|ES|SJ|SE|CH|TJ|TR|TM|UA|GB|UZ|AI|AG|AW|BS|BZ|BB|BM|CA|KY|CR|CU|DM|DO|SV|GD|GP|HT|GT|HN|JM|MQ|MX|UM|MS|NI|AN|PA|PR|KN|LC|PM|VC|TT|TC|US|VG|VI|AS|AU|CK|FJ|PF|GU|KI|MH|FM|NR|NC|NZ|NU|NF|MP|PW|PG|PN|WS|SB|TK|TO|VU|TV|WF|AR|BO|BR|CL|CO|EC|FK|GF|GY|PY|PE|SR|UY|VE'
      }
    ]
  },

  releaseNameMatch: {
    'metadata': {
      'source': path.resolve(__dirname, '..', '..', 'data-tests', 'input-files', 'categories', 'test.tsv'),
      'artist_blacklist': 'test',
      'keyword_blacklist': 'release-name-match',
      'duplicates_threshold': 1,
      'various_artists_threshold': 0,
      'lang': 'en-US',
      'status': 1,
      'time': 1519073116716
    },
    'dataset': [
      {
        'release_name': 'release-name-match',
        'release_meta_language': 'English',
        'orchard_artist': 'Gay and Terry Woods',
        'artist_country': 'United Kingdom',
        'subaccount_name': '',
        'artist_url': '',
        'release_artists_primary_artist': 'Gay and Terry Woods',
        'release_artists_featuring': '',
        'release_artists_remixer': '',
        'release_artists_composer': '',
        'release_artists_orchestra': '',
        'release_artists_ensemble': '',
        'release_artists_conductor': '',
        'release_date': '1/1/1978',
        'sale_start_date': '11/15/2016',
        'itunes_preorder': '',
        'itunes_preorder_date': '',
        'preorder_preview': '',
        'release_itunes_pricing': 'Mid/Front',
        'release_amazon_pricing': 'Frontline',
        'format': 'Full Length',
        'imprint': 'Cooking Vinyl',
        'genre': 'Folk',
        'sub_genre': 'Folk',
        'copyright_information': '1978 Cooking Vinyl',
        'digital_upc': '',
        'manufacturers_upc': '711297201925',
        'label_catalog_number': 'GUMBOCD019',
        'release_version': '',
        'file_name': '711297201925_1_5.wav',
        'volume': '1',
        'track_no': '5',
        'track_name': 'The Reward',
        'meta_language': 'English',
        'version': '',
        'track_artist': 'Gay and Terry Woods',
        'track_artist_featuring': '',
        'track_artist_remixer': '',
        'track_artist_composer': '',
        'track_artist_orchestra': '',
        'track_artist_ensemble': '',
        'track_artist_conductor': '',
        'track_itunes_pricing': 'Front',
        'track_amazon_pricing': 'Frontline',
        'explicit': 'No',
        'isrc': 'GBCEJ7800028',
        'third_party_publisher': 'No',
        'p_information': '1978 Cooking Vinyl',
        'songwriters': '0',
        'publishers': '',
        'only_include_exclude': 'Only Include',
        'territories': 'DZ|AO|BJ|BW|BF|BI|CM|CV|CF|TD|KM|CG|CD|CI|DJ|EG|GQ|ER|ET|GA|GM|GH|GN|GW|KE|LS|LR|LY|MG|MW|ML|MU|MR|MA|MZ|NA|NE|NG|RW|ST|SN|SL|SC|SO|ZA|SZ|SD|TZ|TG|TN|UG|ZM|EH|ZW|AQ|BV|TF|HM|GS|AF|AZ|BH|BD|BT|IO|BN|KH|CN|CX|CC|HK|CC|IN|ID|IR|IQ|JP|JO|KW|LA|LB|MO|MY|MV|MN|MM|NP|KP|OM|PK|PS|PH|QA|SA|SG|KR|LK|SY|TW|TH|TL|AE|VN|YE|AX|AL|AD|AM|AT|BY|BE|BA|BG|HR|CY|CZ|DK|EE|FO|FI|FR|GE|DE|GL|GI|GR|GG|VA|HU|IS|IE|IM|IL|IT|JE|KZ|KG|LV|LI|LT|LU|MK|MT|YT|MD|MC|NL|NO|PL|PT|RE|RO|RU|SH|SM|CS|SK|SI|ES|SJ|SE|CH|TJ|TR|TM|UA|GB|UZ|AI|AG|AW|BS|BZ|BB|BM|CA|KY|CR|CU|DM|DO|SV|GD|GP|HT|GT|HN|JM|MQ|MX|UM|MS|NI|AN|PA|PR|KN|LC|PM|VC|TT|TC|US|VG|VI|AS|AU|CK|FJ|PF|GU|KI|MH|FM|NR|NC|NZ|NU|NF|MP|PW|PG|PN|WS|SB|TK|TO|VU|TV|WF|AR|BO|BR|CL|CO|EC|FK|GF|GY|PY|PE|SR|UY|VE'
      }
    ]
  },

  trackNameMatch: {
    'metadata': {
      'source': path.resolve(__dirname, '..', '..', 'data-tests', 'input-files', 'categories', 'test.tsv'),
      'artist_blacklist': 'test',
      'keyword_blacklist': 'track-name-match',
      'duplicates_threshold': 1,
      'various_artists_threshold': 0,
      'lang': 'en-US',
      'status': 1,
      'time': 1519073116716
    },
    'dataset': [
      {
        'release_name': 'Tender Hooks',
        'release_meta_language': 'English',
        'orchard_artist': 'Gay and Terry Woods',
        'artist_country': 'United Kingdom',
        'subaccount_name': '',
        'artist_url': '',
        'release_artists_primary_artist': 'Gay and Terry Woods',
        'release_artists_featuring': '',
        'release_artists_remixer': '',
        'release_artists_composer': '',
        'release_artists_orchestra': '',
        'release_artists_ensemble': '',
        'release_artists_conductor': '',
        'release_date': '1/1/1978',
        'sale_start_date': '11/15/2016',
        'itunes_preorder': '',
        'itunes_preorder_date': '',
        'preorder_preview': '',
        'release_itunes_pricing': 'Mid/Front',
        'release_amazon_pricing': 'Frontline',
        'format': 'Full Length',
        'imprint': 'Cooking Vinyl',
        'genre': 'Folk',
        'sub_genre': 'Folk',
        'copyright_information': '1978 Cooking Vinyl',
        'digital_upc': '',
        'manufacturers_upc': '711297201925',
        'label_catalog_number': 'GUMBOCD019',
        'release_version': '',
        'file_name': '711297201925_1_6.wav',
        'volume': '1',
        'track_no': '6',
        'track_name': 'track-name-match',
        'meta_language': 'English',
        'version': '',
        'track_artist': 'Gay and Terry Woods',
        'track_artist_featuring': '',
        'track_artist_remixer': '',
        'track_artist_composer': '',
        'track_artist_orchestra': '',
        'track_artist_ensemble': '',
        'track_artist_conductor': '',
        'track_itunes_pricing': 'Front',
        'track_amazon_pricing': 'Frontline',
        'explicit': 'No',
        'isrc': 'GBCEJ7800029',
        'third_party_publisher': 'No',
        'p_information': '1978 Cooking Vinyl',
        'songwriters': '0',
        'publishers': '',
        'only_include_exclude': 'Only Include',
        'territories': 'DZ|AO|BJ|BW|BF|BI|CM|CV|CF|TD|KM|CG|CD|CI|DJ|EG|GQ|ER|ET|GA|GM|GH|GN|GW|KE|LS|LR|LY|MG|MW|ML|MU|MR|MA|MZ|NA|NE|NG|RW|ST|SN|SL|SC|SO|ZA|SZ|SD|TZ|TG|TN|UG|ZM|EH|ZW|AQ|BV|TF|HM|GS|AF|AZ|BH|BD|BT|IO|BN|KH|CN|CX|CC|HK|CC|IN|ID|IR|IQ|JP|JO|KW|LA|LB|MO|MY|MV|MN|MM|NP|KP|OM|PK|PS|PH|QA|SA|SG|KR|LK|SY|TW|TH|TL|AE|VN|YE|AX|AL|AD|AM|AT|BY|BE|BA|BG|HR|CY|CZ|DK|EE|FO|FI|FR|GE|DE|GL|GI|GR|GG|VA|HU|IS|IE|IM|IL|IT|JE|KZ|KG|LV|LI|LT|LU|MK|MT|YT|MD|MC|NL|NO|PL|PT|RE|RO|RU|SH|SM|CS|SK|SI|ES|SJ|SE|CH|TJ|TR|TM|UA|GB|UZ|AI|AG|AW|BS|BZ|BB|BM|CA|KY|CR|CU|DM|DO|SV|GD|GP|HT|GT|HN|JM|MQ|MX|UM|MS|NI|AN|PA|PR|KN|LC|PM|VC|TT|TC|US|VG|VI|AS|AU|CK|FJ|PF|GU|KI|MH|FM|NR|NC|NZ|NU|NF|MP|PW|PG|PN|WS|SB|TK|TO|VU|TV|WF|AR|BO|BR|CL|CO|EC|FK|GF|GY|PY|PE|SR|UY|VE'
      }
    ]
  },

  subStringMatch: {
    'metadata': {
      'source': path.resolve(__dirname, '..', '..', 'data-tests', 'input-files', 'categories', 'test.tsv'),
      'artist_blacklist': 'test',
      'keyword_blacklist': 'I-AM-THE-SUBSTRING',
      'duplicates_threshold': 1,
      'various_artists_threshold': 0,
      'lang': 'en-US',
      'status': 1,
      'time': 1519073116716
    },
    'dataset': [
      {
        'release_name': 'Tender I-AM-THE-SUBSTRING Hooks',
        'release_meta_language': 'English',
        'orchard_artist': 'Gay and Terry Woods',
        'artist_country': 'United Kingdom',
        'subaccount_name': '',
        'artist_url': '',
        'release_artists_primary_artist': 'Gay and Terry Woods',
        'release_artists_featuring': '',
        'release_artists_remixer': '',
        'release_artists_composer': '',
        'release_artists_orchestra': '',
        'release_artists_ensemble': '',
        'release_artists_conductor': '',
        'release_date': '1/1/1978',
        'sale_start_date': '11/15/2016',
        'itunes_preorder': '',
        'itunes_preorder_date': '',
        'preorder_preview': '',
        'release_itunes_pricing': 'Mid/Front',
        'release_amazon_pricing': 'Frontline',
        'format': 'Full Length',
        'imprint': 'Cooking Vinyl',
        'genre': 'Folk',
        'sub_genre': 'Folk',
        'copyright_information': '1978 Cooking Vinyl',
        'digital_upc': '',
        'manufacturers_upc': '711297201925',
        'label_catalog_number': 'GUMBOCD019',
        'release_version': '',
        'file_name': '711297201925_1_7.wav',
        'volume': '1',
        'track_no': '7',
        'track_name': "I Don't See You",
        'meta_language': 'English',
        'version': '',
        'track_artist': 'Gay and Terry Woods',
        'track_artist_featuring': '',
        'track_artist_remixer': '',
        'track_artist_composer': '',
        'track_artist_orchestra': '',
        'track_artist_ensemble': '',
        'track_artist_conductor': '',
        'track_itunes_pricing': 'Front',
        'track_amazon_pricing': 'Frontline',
        'explicit': 'No',
        'isrc': 'GBCEJ7800030',
        'third_party_publisher': 'No',
        'p_information': '1978 Cooking Vinyl',
        'songwriters': '0',
        'publishers': '',
        'only_include_exclude': 'Only Include',
        'territories': 'DZ|AO|BJ|BW|BF|BI|CM|CV|CF|TD|KM|CG|CD|CI|DJ|EG|GQ|ER|ET|GA|GM|GH|GN|GW|KE|LS|LR|LY|MG|MW|ML|MU|MR|MA|MZ|NA|NE|NG|RW|ST|SN|SL|SC|SO|ZA|SZ|SD|TZ|TG|TN|UG|ZM|EH|ZW|AQ|BV|TF|HM|GS|AF|AZ|BH|BD|BT|IO|BN|KH|CN|CX|CC|HK|CC|IN|ID|IR|IQ|JP|JO|KW|LA|LB|MO|MY|MV|MN|MM|NP|KP|OM|PK|PS|PH|QA|SA|SG|KR|LK|SY|TW|TH|TL|AE|VN|YE|AX|AL|AD|AM|AT|BY|BE|BA|BG|HR|CY|CZ|DK|EE|FO|FI|FR|GE|DE|GL|GI|GR|GG|VA|HU|IS|IE|IM|IL|IT|JE|KZ|KG|LV|LI|LT|LU|MK|MT|YT|MD|MC|NL|NO|PL|PT|RE|RO|RU|SH|SM|CS|SK|SI|ES|SJ|SE|CH|TJ|TR|TM|UA|GB|UZ|AI|AG|AW|BS|BZ|BB|BM|CA|KY|CR|CU|DM|DO|SV|GD|GP|HT|GT|HN|JM|MQ|MX|UM|MS|NI|AN|PA|PR|KN|LC|PM|VC|TT|TC|US|VG|VI|AS|AU|CK|FJ|PF|GU|KI|MH|FM|NR|NC|NZ|NU|NF|MP|PW|PG|PN|WS|SB|TK|TO|VU|TV|WF|AR|BO|BR|CL|CO|EC|FK|GF|GY|PY|PE|SR|UY|VE'
      }
    ]
  },

  exactMatch: {
    'metadata': {
      'source': path.resolve(__dirname, '..', '..', 'data-tests', 'input-files', 'categories', 'test.tsv'),
      'artist_blacklist': 'test',
      'keyword_blacklist': 'I-AM-THE-SUBSTRING',
      'duplicates_threshold': 1,
      'various_artists_threshold': 0,
      'lang': 'en-US',
      'status': 1,
      'time': 1519073116716
    },
    'dataset': [
      {
        'release_name': 'I-AM-THE-SUBSTRING',
        'release_meta_language': 'English',
        'orchard_artist': 'Gay and Terry Woods',
        'artist_country': 'United Kingdom',
        'subaccount_name': '',
        'artist_url': '',
        'release_artists_primary_artist': 'Gay and Terry Woods',
        'release_artists_featuring': '',
        'release_artists_remixer': '',
        'release_artists_composer': '',
        'release_artists_orchestra': '',
        'release_artists_ensemble': '',
        'release_artists_conductor': '',
        'release_date': '1/1/1978',
        'sale_start_date': '11/15/2016',
        'itunes_preorder': '',
        'itunes_preorder_date': '',
        'preorder_preview': '',
        'release_itunes_pricing': 'Mid/Front',
        'release_amazon_pricing': 'Frontline',
        'format': 'Full Length',
        'imprint': 'Cooking Vinyl',
        'genre': 'Folk',
        'sub_genre': 'Folk',
        'copyright_information': '1978 Cooking Vinyl',
        'digital_upc': '',
        'manufacturers_upc': '711297201925',
        'label_catalog_number': 'GUMBOCD019',
        'release_version': '',
        'file_name': '711297201925_1_8.wav',
        'volume': '1',
        'track_no': '8',
        'track_name': 'Piece of Summer',
        'meta_language': 'English',
        'version': '',
        'track_artist': 'Gay and Terry Woods',
        'track_artist_featuring': '',
        'track_artist_remixer': '',
        'track_artist_composer': '',
        'track_artist_orchestra': '',
        'track_artist_ensemble': '',
        'track_artist_conductor': '',
        'track_itunes_pricing': 'Front',
        'track_amazon_pricing': 'Frontline',
        'explicit': 'No',
        'isrc': 'GBCEJ7800031',
        'third_party_publisher': 'No',
        'p_information': '1978 Cooking Vinyl',
        'songwriters': '0',
        'publishers': '',
        'only_include_exclude': 'Only Include',
        'territories': 'DZ|AO|BJ|BW|BF|BI|CM|CV|CF|TD|KM|CG|CD|CI|DJ|EG|GQ|ER|ET|GA|GM|GH|GN|GW|KE|LS|LR|LY|MG|MW|ML|MU|MR|MA|MZ|NA|NE|NG|RW|ST|SN|SL|SC|SO|ZA|SZ|SD|TZ|TG|TN|UG|ZM|EH|ZW|AQ|BV|TF|HM|GS|AF|AZ|BH|BD|BT|IO|BN|KH|CN|CX|CC|HK|CC|IN|ID|IR|IQ|JP|JO|KW|LA|LB|MO|MY|MV|MN|MM|NP|KP|OM|PK|PS|PH|QA|SA|SG|KR|LK|SY|TW|TH|TL|AE|VN|YE|AX|AL|AD|AM|AT|BY|BE|BA|BG|HR|CY|CZ|DK|EE|FO|FI|FR|GE|DE|GL|GI|GR|GG|VA|HU|IS|IE|IM|IL|IT|JE|KZ|KG|LV|LI|LT|LU|MK|MT|YT|MD|MC|NL|NO|PL|PT|RE|RO|RU|SH|SM|CS|SK|SI|ES|SJ|SE|CH|TJ|TR|TM|UA|GB|UZ|AI|AG|AW|BS|BZ|BB|BM|CA|KY|CR|CU|DM|DO|SV|GD|GP|HT|GT|HN|JM|MQ|MX|UM|MS|NI|AN|PA|PR|KN|LC|PM|VC|TT|TC|US|VG|VI|AS|AU|CK|FJ|PF|GU|KI|MH|FM|NR|NC|NZ|NU|NF|MP|PW|PG|PN|WS|SB|TK|TO|VU|TV|WF|AR|BO|BR|CL|CO|EC|FK|GF|GY|PY|PE|SR|UY|VE'
      }
    ]
  },

  valid: {
    'metadata': {
      'source': path.resolve(__dirname, '..', '..', 'data-tests', 'input-files', 'categories', 'test.tsv'),
      'artist_blacklist': 'test',
      'keyword_blacklist': 'I-AM-THE-SUBSTRING',
      'duplicates_threshold': 1,
      'various_artists_threshold': 0,
      'lang': 'en-US',
      'status': 1,
      'time': 1519073116716
    },
    'dataset': [
      {
        'release_name': 'Tender Hooks',
        'release_meta_language': 'English',
        'orchard_artist': 'Gay and Terry Woods',
        'artist_country': 'United Kingdom',
        'subaccount_name': '',
        'artist_url': '',
        'release_artists_primary_artist': 'Gay and Terry Woods',
        'release_artists_featuring': '',
        'release_artists_remixer': '',
        'release_artists_composer': '',
        'release_artists_orchestra': '',
        'release_artists_ensemble': '',
        'release_artists_conductor': '',
        'release_date': '1/1/1978',
        'sale_start_date': '11/15/2016',
        'itunes_preorder': '',
        'itunes_preorder_date': '',
        'preorder_preview': '',
        'release_itunes_pricing': 'Mid/Front',
        'release_amazon_pricing': 'Frontline',
        'format': 'Full Length',
        'imprint': 'Cooking Vinyl',
        'genre': 'Folk',
        'sub_genre': 'Folk',
        'copyright_information': '1978 Cooking Vinyl',
        'digital_upc': '',
        'manufacturers_upc': '711297201925',
        'label_catalog_number': 'GUMBOCD019',
        'release_version': '',
        'file_name': '711297201925_1_9.wav',
        'volume': '1',
        'track_no': '9',
        'track_name': 'Lonesome Blue',
        'meta_language': 'English',
        'version': '',
        'track_artist': 'Gay and Terry Woods',
        'track_artist_featuring': '',
        'track_artist_remixer': '',
        'track_artist_composer': '',
        'track_artist_orchestra': '',
        'track_artist_ensemble': '',
        'track_artist_conductor': '',
        'track_itunes_pricing': 'Front',
        'track_amazon_pricing': 'Frontline',
        'explicit': 'No',
        'isrc': 'GBCEJ7800032',
        'third_party_publisher': 'No',
        'p_information': '1978 Cooking Vinyl',
        'songwriters': '0',
        'publishers': '',
        'only_include_exclude': 'Only Include',
        'territories': 'DZ|AO|BJ|BW|BF|BI|CM|CV|CF|TD|KM|CG|CD|CI|DJ|EG|GQ|ER|ET|GA|GM|GH|GN|GW|KE|LS|LR|LY|MG|MW|ML|MU|MR|MA|MZ|NA|NE|NG|RW|ST|SN|SL|SC|SO|ZA|SZ|SD|TZ|TG|TN|UG|ZM|EH|ZW|AQ|BV|TF|HM|GS|AF|AZ|BH|BD|BT|IO|BN|KH|CN|CX|CC|HK|CC|IN|ID|IR|IQ|JP|JO|KW|LA|LB|MO|MY|MV|MN|MM|NP|KP|OM|PK|PS|PH|QA|SA|SG|KR|LK|SY|TW|TH|TL|AE|VN|YE|AX|AL|AD|AM|AT|BY|BE|BA|BG|HR|CY|CZ|DK|EE|FO|FI|FR|GE|DE|GL|GI|GR|GG|VA|HU|IS|IE|IM|IL|IT|JE|KZ|KG|LV|LI|LT|LU|MK|MT|YT|MD|MC|NL|NO|PL|PT|RE|RO|RU|SH|SM|CS|SK|SI|ES|SJ|SE|CH|TJ|TR|TM|UA|GB|UZ|AI|AG|AW|BS|BZ|BB|BM|CA|KY|CR|CU|DM|DO|SV|GD|GP|HT|GT|HN|JM|MQ|MX|UM|MS|NI|AN|PA|PR|KN|LC|PM|VC|TT|TC|US|VG|VI|AS|AU|CK|FJ|PF|GU|KI|MH|FM|NR|NC|NZ|NU|NF|MP|PW|PG|PN|WS|SB|TK|TO|VU|TV|WF|AR|BO|BR|CL|CO|EC|FK|GF|GY|PY|PE|SR|UY|VE'
      }
    ]
  }

}
